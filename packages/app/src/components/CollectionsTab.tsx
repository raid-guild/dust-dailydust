...
export function CollectionsTab() {
  ...
  const {
    ...
    error: chainError,
    refetch,
  } = useOnchainNotes({ limit: 300, offset: 0 });
  ...
  const candidates = useMemo(() => {
    const map = new Map<
      string,
      { id: string; title: string; tags: string[]; updatedAt: number }
    >();
    for (const n of chainNotes) {
      map.set(n.id, {
        ...
        updatedAt: Number((n as any).updatedAt ?? 0),
      });
    }
      ...
      <div className="space-y-2">
        ...
        <div className="rounded border border-neutral-300 p-3 space-y-2 bg-panel">
          <div className="flex items-center justify-between">
            <div className="uppercase tracking-[.2em] text-xs text-neutral-500 font-accent">
              Add published notes
            </div>
            <span className="text-xs text-text-secondary">
              {newNoteIds.length} selected
            </span>
          </div>
          ...
          {newNoteIds.length > 0 && (
            <div className="pt-2 border-t border-neutral-200">
              ...
              <ul className="space-y-1">
                {newNoteIds.map((nid) => {
                  const n =
                    chainNotes.find((nn) => nn.id === nid) ||
                    localNotes.find((nn) => nn.id === nid);
        ...
        <div className="space-y-4">
          {collections.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className="border border-neutral-300 rounded p-3 space-y-3 bg-panel"
              >
                ...
                {col.noteIds.length === 0 ? (
                  ...
                  <ul className="space-y-1">
                    {col.noteIds.map((nid, idx) => {
                      const note =
                        localNotes.find((n) => n.id === nid) ||
                        chainNotes.find((n) => n.id === nid);
                      return (
                        <li
                          key={nid}
                          className="flex items-center justify-between gap-2 border border-neutral-300 rounded px-2 py-1 bg-panel"
                        >
                          <div className="text-sm truncate text-text-primary">
                            {idx + 1}.{" "}
                            {note ? (note as any).title : `(missing) ${nid}`}
                          </div>
                          <div className="flex items-center gap-2">
                            {idx > 0 && (
                              <button
