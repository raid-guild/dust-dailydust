...
export function PublishWizard({
  ...
  const [addStepWaypointId, setAddStepWaypointId] = useState<string | null>(
    null
  );

  // Moved up: hooks used by effects below
  const { data: dustClient } = useDustClient();
  const { createDraft, updateDraftImmediate, deleteDraft, getDraft } =
    useDrafts();
  ...
  useEffect(() => {
    const id =
      contentState.effectiveDraftId || wizardDraftId || draftId || null;
    if (!id) return;
    const d = getDraft(id);
    if (!d) return;
    if (selectedWaypointId === null && d.selectedWaypointId)
      setSelectedWaypointId(d.selectedWaypointId);
    if (
      routeSteps.length === 0 &&
      Array.isArray(d.routeSteps) &&
      d.routeSteps.length > 0
    )
      setRouteSteps(d.routeSteps as any);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [contentState.effectiveDraftId, wizardDraftId, draftId]);

  const canContinueFromStep1 = useMemo(() => {
    return (
      contentState.title.trim().length > 0 &&
      contentState.content.trim().length > 0
    );
  ...
  const handleDone = async () => {
    ...
    try {
      ...
      const noteHexId: `0x${string}` =
        contentState.noteId &&
        contentState.noteId.startsWith("0x") &&
        contentState.noteId.length === 66
          ? (contentState.noteId as `0x${string}`)
          : randomBytes32();

      const noteSystemId = resourceToHex({
        type: "system",
        namespace: NAMESPACE,
        name: "NoteSystem",
      });
      ...
      if (hex32.test(wp.entityId)) {
        ...
        alert(
          "Selected waypoint must have a valid bytes32 entityId or coordinates"
        );
      ...
      if (routeSteps.length > 0) {
        const waypointSystemId = resourceToHex({
          type: "system",
          namespace: NAMESPACE,
          name: "WaypointSystem",
        });
        ...
        await (dustClient as any).provider.request({
          ...
          params: [
            {
              systemId: waypointSystemId,
              abi: waypointSystemAbi,
              functionName: "createWaypointGroup",
              args: [noteHexId, groupId, groupName, color, isPublic],
            },
          ],
        });
            ...
            params: [
              {
                systemId: waypointSystemId,
                abi: waypointSystemAbi,
                functionName: "addWaypointStep",
                args: [noteHexId, groupId, i + 1, x2, y2, z2, label],
              },
            ],
          });
        }
      ...
      <CardHeader>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div
              className={`px-2 py-1 text-xs rounded-full border ${step === 1 ? "bg-brand-600 text-white border-brand-600" : "bg-neutral-100 text-text-secondary border-neutral-300"}`}
            >
              1. Content
            </div>
            <div
              className={`px-2 py-1 text-xs rounded-full border ${step === 2 ? "bg-brand-600 text-white border-brand-600" : "bg-neutral-100 text-text-secondary border-neutral-300"}`}
            >
              2. Location
            </div>
            <div
              className={`px-2 py-1 text-xs rounded-full border ${step === 3 ? "bg-brand-600 text-white border-brand-600" : "bg-neutral-100 text-text-secondary border-neutral-300"}`}
            >
              3. Route (optional)
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            {step < 3 && (
              <button
                onClick={() =>
                  setStep((s) => Math.min(3, (s as number) + 1) as Step)
                }
                disabled={
            ...
            <button
              onClick={onCancel}
              className="px-3 py-1.5 text-sm text-text-secondary bg-neutral-100 rounded hover:bg-neutral-200"
            >
              Cancel
            </button>
          </div>
      ...
      <CardContent>
        ...
        {step === 2 && (
          <div className="space-y-4">
            <p className="text-sm text-text-secondary">
              Choose the single location where this story lives.
            </p>
            <WaypointPicker
              waypoints={waypoints}
              selectedId={selectedWaypointId}
              onChange={setSelectedWaypointId}
            />
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-5">
            <p className="text-sm text-text-secondary">
              Optionally define a route/group with multiple stops.
            </p>
            ...
            {routeSteps.length === 0 ? (
              <div className="text-sm text-text-secondary">
                No steps yet. Add waypoints to build a route.
              </div>
            ) : (
              <ol className="space-y-2">
                {routeSteps.map((s, idx) => {
                  ...
                  return (
                    <li
                      key={`${s.waypointId}-${idx}`}
                      className="p-3 rounded border border-neutral-300 dark:border-neutral-800 flex items-start gap-3"
                    >
                      <div className="text-xs w-6 h-6 flex items-center justify-center rounded-full bg-brand-600 text-white">
                        {idx + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="font-medium">
                          {wp?.name || s.waypointId}
                        </div>
                        ...
                        <input
                          value={s.label ?? ""}
                          onChange={(e) =>
                            updateRouteStepLabel(idx, e.target.value)
                          }
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <button
                          onClick={() => moveRouteStep(idx, 1)}
                          disabled={idx === routeSteps.length - 1}
                          className="px-2 py-1 text-xs bg-neutral-100 rounded disabled:opacity-50"
                        >
                          ↓
                        </button>
                        <button
                          ...
                          className="px-2 py-1 text-xs bg-danger/10 text-danger rounded"
                        >
                          Remove
                        </button>
                      </div>
...
function WaypointPicker({
  ...
  return (
    <div className="space-y-2">
      {waypoints.length === 0 ? (
        <div className="text-sm text-text-secondary">
          No waypoints available. Create one in Waypoint Tools.
        </div>
      ) : (
        <ul className="space-y-2">
          {waypoints.map((wp) => (
            <li
              key={wp.id}
              className={`p-3 rounded border ${selectedId === wp.id ? "border-brand-500 bg-brand-50" : "border-neutral-300"}`}
            >
