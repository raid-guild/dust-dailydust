...
export function useCollections() {
      ...
      if (raw) {
        ...
        const normalized: Collection[] = Array.isArray(parsed)
          ? parsed.map((c: any) => ({
              ...
              title: typeof c?.title === "string" ? c.title : "Untitled Collection",
              ...
              publishedAt: typeof c?.publishedAt === "number" ? c.publishedAt : undefined,
              createdAt: typeof c?.createdAt === "number" ? c.createdAt : Date.now(),
              updatedAt: typeof c?.updatedAt === "number" ? c.updatedAt : Date.now(),
              noteIds: Array.isArray(c?.noteIds) ? c.noteIds.filter((n: any) => typeof n === "string") : [],
            }))
          : [];
  ...
  // CRUD
  const createCollection = (init?: Partial<Omit<Collection, "id" | "createdAt" | "updatedAt">>) => {
    const now = Date.now();
  ...
  const removeNoteFromCollection = (collectionId: string, noteId: string) => {
    setCollections(prev => prev.map(c => (c.id === collectionId ? { ...c, noteIds: c.noteIds.filter(id => id !== noteId), updatedAt: Date.now() } : c)));
  };

  const reorderNotesInCollection = (collectionId: string, fromIndex: number, toIndex: number) => {
    setCollections(prev => prev.map(c => {
      if (c.id !== collectionId) return c;
  ...
  const getPublished = () => collections.filter(c => typeof c.publishedAt === "number");
  const getDrafts = () => collections.filter(c => typeof c.publishedAt !== "number");
  const getByTag = (tag: string) => collections.filter(c => c.tags?.includes(tag));
  ...
  const withNotes = (id: string, notes: Note[]) => {
    ...
    const items = c.noteIds.map(nid => map.get(nid)).filter(Boolean) as Note[];
    return { ...c, notes: items } as const;
  };

  // Export/Import for collections
  const exportCollections = () => ({ version: 1, exportedAt: new Date().toISOString(), collections });
  const importCollections = (input: unknown, replace = false) => {
    let payload: any = input as any;
    if (typeof payload === "string") {
      try { payload = JSON.parse(payload); } catch { payload = []; }
    }
    const arr = Array.isArray(payload?.collections) ? payload.collections : Array.isArray(payload) ? payload : [];
    const sanitize = (item: any): Collection | null => {
      if (!item || typeof item !== "object") return null;
      const title = typeof item.title === "string" ? item.title : "Untitled Collection";
      const description = typeof item.description === "string" ? item.description : "";
      const coverImageUrl = typeof item.coverImageUrl === "string" ? item.coverImageUrl : undefined;
      ...
      const publishedAt = typeof item.publishedAt === "number" ? item.publishedAt : (typeof item.publishedAt === "string" ? (isNaN(Date.parse(item.publishedAt)) ? undefined : Date.parse(item.publishedAt)) : undefined);
      const createdAt = typeof item.createdAt === "number" ? item.createdAt : Date.now();
      const updatedAt = typeof item.updatedAt === "number" ? item.updatedAt : createdAt;
      const noteIds: string[] = Array.isArray(item.noteIds) ? item.noteIds.filter((n: any) => typeof n === "string") : [];
      const id = typeof item.id === "string" && item.id ? item.id : crypto.randomUUID();
      return { id, title, description, coverImageUrl, tags, category, featured, publishedAt, createdAt, updatedAt, noteIds };
    };

    const sanitized = arr.map(sanitize).filter(Boolean) as Collection[];
    if (sanitized.length === 0) return false;

    if (replace) { setCollections(sanitized); return true; }
  ...
  const stats = useMemo(() => ({
    ...
    published: collections.filter(c => typeof c.publishedAt === "number").length,
    drafts: collections.filter(c => typeof c.publishedAt !== "number").length,
  }), [collections]);
