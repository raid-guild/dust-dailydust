...
library Tipper {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "thedailydust", name: "Tipper", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x74627468656461696c7964757374000054697070657200000000000000000000);
   ...
   */
  function encodeStatic(bool tipped) internal pure returns (bytes memory) {
    return abi.encodePacked(tipped);
  }
   ...
   */
  function encode(bool tipped) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(tipped);
