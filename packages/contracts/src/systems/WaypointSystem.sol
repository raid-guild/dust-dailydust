...
contract WaypointSystem is System {
    ...
    function createWaypointGroup(
        ...
        require(note.owner == _msgSender(), "Only note owner can create waypoint groups");
        
        // Ensure group doesn't exist (check if name is empty)
        require(bytes(WaypointGroup.getName(noteId, groupId)).length == 0, "Waypoint group already exists");
        
        WaypointGroup.set(noteId, groupId, WaypointGroupData({
            color: color,
            isPublic: isPublic,
            name: name
        }));
    ...
    function updateWaypointGroup(
        ...
        // Verify note exists and caller is owner
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only note owner can update waypoint groups");
        
        // Verify group exists
        require(bytes(WaypointGroup.getName(noteId, groupId)).length > 0, "Waypoint group does not exist");
    ...
    function deleteWaypointGroup(bytes32 noteId, uint16 groupId) public {
        // Verify note exists and caller is owner
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only note owner can delete waypoint groups");
    ...
    function addWaypointStep(
        ...
        // Verify note exists and caller is owner
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only note owner can add waypoint steps");
        
        // Verify group exists
        require(bytes(WaypointGroup.getName(noteId, groupId)).length > 0, "Waypoint group does not exist");
        
        // Ensure step doesn't exist at this index (check if label is empty)
        require(bytes(WaypointStep.getLabel(noteId, groupId, index)).length == 0, "Waypoint step already exists at this index");
        
        WaypointStep.set(noteId, groupId, index, WaypointStepData({
            x: x,
            y: y,
            z: z,
            label: label
    ...
    function updateWaypointStep(
        ...
        // Verify note exists and caller is owner
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only note owner can update waypoint steps");
        
        // Verify step exists
        require(bytes(WaypointStep.getLabel(noteId, groupId, index)).length > 0, "Waypoint step does not exist");
    ...
    function removeWaypointStep(
        ...
        // Verify note exists and caller is owner
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only note owner can remove waypoint steps");
