...
contract NoteSystem is System {
    ...
    function createNote(
        ...
        require(Note.getOwner(noteId) == address(0), "Note already exists");
        
        uint64 timestamp = uint64(block.timestamp);
        
        Note.set(noteId, NoteData({
            owner: _msgSender(),
            createdAt: timestamp,
            updatedAt: timestamp,
    ...
    function updateNote(
        ...
        require(note.owner == _msgSender(), "Only owner can update note");
    ...
    function updateHeaderImageUrl(bytes32 noteId, string memory newUrl) public {
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only owner can update header image");
    ...
    function updateTipJar(bytes32 noteId, address newTipJar) public {
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only owner can update tip jar");
        require(newTipJar != address(0), "Invalid tip jar address");
    ...
    function deleteNote(bytes32 noteId) public {
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only owner can delete note");
    ...
    function createNoteLink(
        ...
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only note owner can create links");
    ...
    function removeNoteLink(bytes32 noteId, bytes32 entityId) public {
        // Verify note exists and caller is owner
        NoteData memory note = Note.get(noteId);
        require(note.owner == _msgSender(), "Only note owner can remove links");
